...
# available BLM AIM Riparian and Wetland Points points from years in which at least 100 points were
...
data_path <- "~/Projects/AIM/Data/AIM_Wetland_Pub_4-1-26.gdb/AIM_Wetland_Pub_4-1-26.gdb"
...
all_rw <- sf::st_read(dsn = data_path,
                             layer = "AIM_Wetland__I_Indicators" ) |> dplyr::select(EvaluationID, LatitudeWGS84, LongitudeWGS84, FieldEvalDate)


# Again, the sampling design needs to be stratified by year
# This will extract the year from the FieldEvalDate variable
all_points_sample <- all_rw |>
  dplyr::mutate(YearVisited = lubridate::year(FieldEvalDate)) |>

  # TODO convert to projected coordinates for spsurvey
  sf::st_

# summary
all_points_summary <- all_points_sample |> dplyr::group_by(YearVisited) |> dplyr::count()
...
sample_size_vector <- ceiling(table(all_points_sample$YearVisited) * 0.1)

# Only years in which there were at least 100 points sampled should be included
# in the sampling design
sample_size_vector <- sample_size_vector[table(all_points_sample$YearVisited) >= 100]
...
sample <- spsurvey::grts(sframe = dplyr::filter(all_points_sample,
...
# add Lat/Long
sample_points <- dplyr::left_join(sample_points, all_points |>
                                    dplyr::select(PrimaryKey, Latitude_NAD83, Longitude_NAD83) |> sf::st_drop_geometry())
